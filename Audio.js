...
var MicrophoneInput = function MicrophoneInput(bufferSize) {
	...
	this.meyda = Meyda.createMeydaAnalyzer({
		...
		featureExtractors: ['loudness', 'amplitudeSpectrum', 'buffer'],
...
// MFCC DIST
function DiffOne(mfcc0, mfcc1) {
  const sum0 = Norm2(mfcc0);
  const sum1 = Norm2(mfcc1);
}

function DiffOneVsN(mfcc0, mfccs1) {
  const norm0 = Norm2(mfcc0);
  let ret = [];
  mfccs1.forEach((m) => {
    const norm1 = Norm2(m);
    let sum = 0;
    for (let i=0; i<mfcc0.length; i++) {
      sum += mfcc0[i]*m[i];
    }
    const cost = 1-(sum/(norm1*norm0));
    ret.push(cost);
  })
  return ret;
}

function DiffNVsN(mfccs0, mfccs1) {
  let ret = [];
  let sum = 0;
  mfccs0.forEach((m0) => {
    const x = DiffOneVsN(m0, mfccs1);
    ret.push(x);
    sum += x;
  });
  return ret;
